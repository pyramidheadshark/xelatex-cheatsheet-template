% >>> КОММЕНТАРИЙ: Начало контента шпаргалки. Используйте \section для разделов.
% >>> Если раздел или большой блок под ним переносится неудачно,
...
\red{Pandas} — библиотека Python для \textbf{обработки и анализа данных}. Предоставляет гибкие и мощные структуры данных (Series, DataFrame), разработанные для работы с "реляционными" или "табличными" данными.\footcite[Гл. 5]{mckinney2017python} % Ссылка на библиографию

\bigskip % Вертикальный отступ
...
\textbf{Series}: Одномерный массив с метками (индексом). Похож на столбец таблицы или словарь. \sep % \sep - маленький разделитель
...
% >>> КОММЕНТАРИЙ: Используем кастомный блок myblock и кастомные команды mycommand
\begin{myblock}{Чтение и запись данных}
Pandas поддерживает множество форматов.
% >>> КОММЕНТАРИЙ: \mycommand отображает команду и ее описание
...
% >>> КОММЕНТАРИЙ: Этот блок codebox был очень большим. Если он будет плохо разрываться,
% >>> можно рассмотреть его дальнейшее разделение (например, .loc и .iloc в отдельные блоки)
% >>> или использовать \needspace перед блоком.
...
% >>> КОММЕНТАРИЙ: Выделяем примеры булевой фильтрации в отдельный блок 'myexampleblock'.
% Это улучшает структуру и помогает избежать слишком больших codebox'ов.
...
% >>> КОММЕНТАРИЙ: Исправлено: _ в именах функций внутри текста (не кода!) нужно экранировать: `\_`
...
% >>> КОММЕНТАРИЙ: Этот блок codebox довольно большой. Если возникнут проблемы с разрывом,
% >>> разбейте его на части (например, основы groupby, agg, size/iteration)
% >>> или используйте \needspace перед ним.
...
% >>> КОММЕНТАРИЙ: Исправлено: _ в заголовке нужно экранировать `\_`.
...
% >>> КОММЕНТАРИЙ: Этот блок codebox содержит несколько концепций. Если он плохо разрывается,
% >>> разделите apply, map, applymap на отдельные блоки или используйте \needspace.
...
% >>> КОММЕНТАРИЙ: Вставляем сгенерированное изображение.
% >>> Лог компиляции может показать странное сообщение "Graphic file (type bmp)" для PNG,
% >>> но если изображение отображается корректно, это можно игнорировать.
\mygraphics{img/pandas_plot_example.png}
\end{textbox}

\section{Полезные ссылки и советы / Links \& Tips}
\begin{myblock}{Полезные ресурсы}
% >>> КОММЕНТАРИЙ: Используем команду \resourcelink для форматирования ссылок
...
% >>> КОММЕНТАРИЙ: Используем переопределенное окружение quote для форматирования цитат
\begin{quote}
